...
(function () {	
	...
	var UpdateCheck = function (data) {
		for (var i = 0; i < ModSupport.availableMods.length; i++) {
			...
			if(mod.url == "https://github.com/Turntablelover/Game-Dev-Tycoon-Mod" && mod.version > "1.7.1" && mod.active) {
				var div = $("body");
				...
				div = $("#UpdateContainer");
				div.append('<div id="update" style="margin-left:50px;width: 400px;" >A new update <span style="color:red;">for</span> Game Dev Tycoon: Platforms, Events, and Topics OH MY is now available.</br></br> Please update to 1.7.1 at https://github.com/Turntablelover/Game-Dev-Tycoon-Mod </div>');
				...
				events: [
					{
						...
						getNotification: function (company) {
							return new Notification({
								header: "Industry News".localize(),
								text: "Mirconoft has patched up Vindows 3.0, and the patch's name is Vindows 3.1. Vindows 3.1 just hit RTM, and it will be available in stores {0}.".localize().format(General.getETADescription('8/1/3', '8/2/3')),
								image: icon12
							});
						}
					}
				...
				events: [
					{
						...
						getNotification: function (company) {
							return new Notification({
								header: "Industry News".localize(),
								text: "Samazon, the owner of the popular shopping website, Samazon.com, has just released a new version of their gaming tablet, the Skindle Fire, called the Skindle Fire HD. The Skindle Fire HD will be in stores {0}.".localize().format(General.getETADescription('30/1/3', '31/3/3')),
								image: icon23
							});
						}
					}
		...
		GDT.addPlatform(
			{
				...
				licencePrize: 500000,
				published: '23/2/2',
				platformRetireDate: '90/2/2',
				developmentCosts: 600000,
				genreWeightings: [0.9, 1.0, 1.0, 0.9, 1, 0.9],
				audienceWeightings: [0.9, 1.0, 0.8],
				techLevel: 11,
				iconUri: icon50,
		...
		var DalekInvasion = {
			...
			complete: function (decision) {
				...
				if (decision === 0) {
					...
					n.adjustCash(-100000, "Extermination");
					n.adjustHype(5 - 2500 * company.getRandom());
					n.adjustFans(-100000);
				...
				if (decision === 1) {
					...
					n.adjustCash(100000, "Saved the Day");
		...
		var GamePort = {
			...
			complete: function (decision) {
				...
				if (decision === 0) {
					...
					n.adjustCash(+1000000000, "Sued Ninvento");
		...
		var GamingMarket = {
			...
			complete: function (decision) {
                ...
                if (decision === 2) {
                	var n = new Notification({
                    	header: "Gaming Market".localize(),
                        text: "In The Gaming Market, you will get fans, when your game is good enough. \nFans will like your company if the game is awesome."
                    });
                	n.adjustFans(500);
                	if (company.isGameProgressBetween(0.0, 0.0)) {
                    	n.adjustHype(5 + 25 * company.getRandom());
	...
	GDT.addTopics([
		{
	    	...
	    	genreWeightings: [1, 1, 1, 0.8, 1, 0.9],
			audienceWeightings: [1, 1, 0.6],
		}, {
			id: "Legend of Zelda",
			...
			id: "Exercise",
	    	name: "Exercise".localize("game topic"),
	    	genreWeightings: [0.6, 0.6, 0.6, 1, 0.7, 1],
			audienceWeightings: [1, 1, 0.6],
		}, {
			id: "Animal Crossing",
	...
	venomousmodAPI.addExtendGame = function () {
		...
		VenomOption = document.createElement("option");
		VenomOption.text = "125 Years";
		VenomOption.value = "4.16667";
		element.appendChild(VenomOption);
		
		VenomOption = document.createElement("option");
		VenomOption.text = "130 Years";
		VenomOption.value = "4.25";
		element.appendChild(VenomOption);
		
		VenomOption = document.createElement("option");
		VenomOption.text = "135 Years";
		VenomOption.value = "4.45";
		element.appendChild(VenomOption);
		
		VenomOption = document.createElement("option");
		VenomOption.text = "140 Years";
		VenomOption.value = "4.68";
		element.appendChild(VenomOption);
